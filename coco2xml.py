...
def instance2xml_base(anno):
    ...
    anno_tree = E.annotation(
        E.folder('/{}'.format(anno['category_id'])),
    ...
    for name, groups in cytoolz.groupby('file_name', merged_info_list).items():
        ...
        for group in groups:
            filenames.append(os.path.join(outdir, re.sub(" ", "_", group['category_id']),
                                    os.path.splitext(name.split('/')[1])[0] + ".xml"))
...
if __name__ == "__main__":
    ...
    anno_file = '/media/kisna/nano_ti_data1/DL_git/FLIRrgb_TFODAPI/flirRGB/test/thermal_annotations.json'
    output_dir = '/media/kisna/nano_ti_data1/DL_git/FLIRrgb_TFODAPI/flirRGB/test/test'
    type = 'instance'
    main()
